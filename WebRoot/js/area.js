...
function change_new(v){
	var str="0";
	for(i=0;i<v;i++){
		str+=("_"+(document.getElementById(pro_id[i]).selectedIndex-1));
	};
	var ss=document.getElementById(pro_id[v]);
	with(ss){
		length = 0;
		options[0]=new Option(opt0[v],opt0[v]);
		if(v && document.getElementById(pro_id[v-1]).selectedIndex>0 || !v){
			if(dsy.Exists(str)){
				ar = dsy.Items[str];
				for(i=0;i<ar.length;i++){
					options[length]=new Option(ar[i],ar[i]);
		...
		if(++v<pro_id.length){change_new(v);}
	}//End with
}
var dsy = new Dsy();
...
var pro_id=["province","city","county"];
...
function area(){  //初始化函数
	for(i=0;i<pro_id.length-1;i++){
	  document.getElementById(pro_id[i]).onchange=new Function("change_new("+(i+1)+")");
	}
	change_new(0);
}

/*function _init_area1(){  //初始化函数
	for(i=0;i<new_sele.length-1;i++){
	  document.getElementById(new_sele[i]).onchange=new Function("change("+(i+1)+")");
	}
	change(0);
}*/