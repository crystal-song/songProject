...
//            System.out.println("xmlInfo=" + xmlInfo);   
    ...
    private static String getXmlInfo() {  
      ...
      /*  platformNo 商户编号 Y 商户在易宝唯一标识
        platformUserNo 商户平台会员标识 Y 会员在商户平台唯一标识
        requestNo 请求流水号 Y
        nickName 昵称 N 交易查询时希望显示的会员名称，若不写则
        与会员标识一致
        realName 会员真实姓名 Y 会员真实姓名
        idCardType 身份证类型 Y 【见身份证类型】
        idCardNo 身份证号 Y 会员身份证号
        mobile 手机号 Y 接收短信验证码的手机号
        email 邮箱 Y 邮箱
        callbackUrl 页面回跳URL Y 页面回跳URL
        notifyUrl 服务器通知URL Y 服务器通知URL
        2.1.2. 接口输出
        参数*/
